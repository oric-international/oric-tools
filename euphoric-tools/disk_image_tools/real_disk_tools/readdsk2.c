...
char pad[256];
int drive;
int geometry=1,tracks=0,sectors=0,sides=2;
...
unsigned char trackbuf[6400+512];  
int ptr_track;
unsigned int crc;
...
{
        /* stores the sectors with small gaps, and no index mark */
	store_gap(12,0);
...
{
        int sectmax,i,status;
	if (sectors) sectmax=sectors;
	else switch (sectsize) {
                case 0: sectmax=29; break; /* 128-bytes sectors = 3712/6400 */
                case 1: sectmax=18; break; /* 256-bytes sectors = 4608/6400 */
                case 2: sectmax=10; break; /* 512-bytes sectors = 5120/6400 */
                case 3: sectmax=5; break; /* 1024-bytes sectors = 5120/6400 */
	}
	init_track();
	for (i=1;i<=sectmax;i++) {
		status=read_sector(cyl,head,i);
                if (status==OK) {
                        putchar('.');
                        store_sector(buf,cyl,head,i,sectsize);
                } else if (status==BADCRC) {
                        putchar('x');
                        store_sector_bad_crc(buf,cyl,head,i,sectsize);
                } else {
                        putchar('?');
                        break;
                }
...
int test_track(int track,int side,int *sectsize)
{
	...
	do {
                status=read_sector(track,side,1);
...
int transfer_side(int side, FILE *fd)
{
        int ntrack,i,status;
        int sectsize;
        biosparams[3]=sectsize=1;   /* most often, 256-bytes sectors will be read */
	di.drive=drive;
	_bios_disk(_DISK_RESET,&di);
	...
	if (tracks) ntrack=tracks;
        for (i=0;i<ntrack;i++) {
                ...
                printf("Testing side %d track %2d (sector size: %d) -> %d\n ",side,i,128<<sectsize,status);
		if (status!=OK && status!=BADCRC) return i;
                printf("Reading side %d track %2d (sector size: %d) ",side,i,128<<sectsize);
...
usage()
{
	...
	printf("\t-s:nn (specify number of sectors per track, e.g -s:17)\n");
        printf("Nota: specifying the sectors per track number prevents the program from\n");
        printf("searching inexistent sectors\n");
	exit(1);
}
...
#ifdef DEBUG
printf("biosparams=%x:%x\n",biosparams);
#endif
printf("transfering side 0\n");
	tracks=transfer_side(0,fd);
printf("transfering side 1\n");
