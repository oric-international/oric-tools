...
char *loader_check_crc[] = {
":20010000206ae7a9818d4502a9018d4602a9908d0e03209be520c9e6f032853420c9e6857c",
":200120003320c9e6858320c9e6a84820630168a8adb10248b133852f68a208062f2a90021f",
":2001400049d5cad0f6c8d0eb4583f0c9482054e6a9228d4502a9ee8d4602203de968d06a58",
":200160004cd6e82c0003a9102c0d03f0fba2008e0c032c0003ca8e0803862f8e090358d01e",
":20018000fead0c0349108d0c032c0003ae08038e09032910d00ea52fe06eb004e044d00c41",
":2001a000e09ad010a52fe063b008e0372a0a9008b00de08d2a2ab0079133c8f005a9fe8551",
":0b01c0002f40a9108d0c0368686860d8"
};

char *loader_no_check[] = {
":20010000206ae7a9618d4502a9018d4602a9908d0e03209be520c9e6f011853420c9e685bd",
":200120003320c9e6a82043014c1501a9228d4502a9ee8d4602203de92051e6adb102d06a0d",
":200140004cd6e82c0003a9102c0d03f0fba2008e0c032c0003ca8e0803862f8e090358d03e",
":20016000fead0c0349108d0c032c0003ae08038e09032910d00ea52fe06eb004e044d00c61",
":20018000e09ad010a52fe063b008e0372a0a9008b00de08d2a2ab0079133c8f005a9fe8571",
":0b01a0002f40a9108d0c0368686860f8"
};

int hexa(char c) 
{
  if (c<='9') return c-'0';
  else return ((c-'A')&0x0F)+10;
}
...
void usage()
{
  printf("Usage: tap2cd [-c] <.TAP file> <WAV file>\n");
  printf("(-c check CRC)\n");
  exit(1);
}
...
void emit_fast_page(int adr,int size)
{
  ...
  if (check_crc)	/* give enough time to compute the crc */
...
void emit_loader()
{
  ...
  if (check_crc) {
    nb_lines = sizeof(loader_check_crc) / sizeof(char *);
    loader = loader_check_crc;
  }

  for (n=0; n<nb_lines; n++) {
    char *line = loader[n];
    for (i=9; i<strlen(line)-2; i+=2) {
      int byte = hexa(line[i])*16 + hexa(line[i+1]);
      Mem[addr] = byte;
      addr++;
    }
  }
  loader_header[5] = (addr-1) & 0xFF;
  emit_standard_header(loader_header);
  emit_standard_gap(30);
  for (i=0x100; i<addr; i++)
      emit_standard_byte(Mem[i]);
}
